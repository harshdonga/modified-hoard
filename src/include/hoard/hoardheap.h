...
namespace Hoard {
  ...
  public:
    inline static bool function (unsigned int u,
                                unsigned int a,
                                size_t objSize,
                                bool isAggressiveReclaim)
    {
      /*
        Returns 1 iff we've crossed the emptiness threshold:
        
        If aggressive reclaiming is enabled, we might want to consider a superblock
        empty if it has more free space than usual, to reclaim it more aggressively.
        
        The original condition was:
        U < A - 2S   &&   U < EMPTINESS_CLASSES-1/EMPTINESS_CLASSES * A
        
        Let's make it more aggressive by reducing the threshold when isAggressiveReclaim is true.
      */
      
      if (isAggressiveReclaim) {
        return ((EMPTINESS_CLASSES * u) < ((EMPTINESS_CLASSES-1) * a)) && ((u < a - (3 * SUPERBLOCK_SIZE) / objSize));
      } else {
        // Original logic for when aggressive reclaim is not enabled
        return ((EMPTINESS_CLASSES * u) < ((EMPTINESS_CLASSES-1) * a)) && ((u < a - (2 * SUPERBLOCK_SIZE) / objSize));
      }
    }
  };
